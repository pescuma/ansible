...
short_description: Manage Jenkins slaves

description:
    - The C(jenkins_slave) module allows to manage Jenkins slaves using the Jenkins REST API.
    - All the work is done in the Jenkins master, so it does not matter where this module runs.
...
options:
    ...
    server_validate_certs:
        description:
            - If Jenkins server certificates should be validated.
        required: false
        type: bool
        default: true
...
- name: Query slave state
  jenkins_slave:
    name: slave_name
    state: query
    server_url: http://jenkins_server:8080
    server_username: ansible_user
    server_token: 1234567890123456789012345678901234567890
...
# All the work is done on the jenkins server, so it does not matter much if you delegate the work.
